...
export const createInochi2DController = async ({ wasmUrl, debug }) => {
  ...
  const targetOpen = clamp01(lipSyncLayer.targetOpen);
  const currentOpen = clamp01(lipSyncLayer.currentOpen);
  ...
  const mix =
    smoothingMs <= 0
      ...
      : Math.min(
          1,
          Math.max(0, deltaTimeMs / smoothingMs),
        );

  const nextOpen =
    currentOpen + (targetOpen - currentOpen) * mix;

  const shouldRemainActive =
    targetOpen > LIP_SYNC_CLOSE_EPSILON ||
    nextOpen > LIP_SYNC_CLOSE_EPSILON;

  const finalOpen = shouldRemainActive ? nextOpen : 0;
  const finalViseme =
    finalOpen > LIP_SYNC_CLOSE_EPSILON
      ? lipSyncLayer.viseme
      : 'neutral';

  const pose = resolveLipSyncPose(
    finalOpen,
    finalViseme,
  );
  ...
  lipSyncLayer = {
    ...lipSyncLayer,
    targetOpen: shouldRemainActive ? targetOpen : 0,
  ...
  if (canvas) {
    canvas.dataset.inochi2dMouthShape =
      `${pose[0].toFixed(3)},${pose[1].toFixed(3)}`;
  ...
  console.log(
    parameters.map((parameter) => parameter?.name).filter(Boolean)
  );
  ...
  for (const parameter of parameters) {
    if (!parameter || typeof parameter.name !== 'string') {
      ...
      if (parameter.name === "Mouth:: Shape") {
       console.log("MOUTH PARAMETER:", parameter);
　　　　}

      const defaults =
  parameter.name.replace(/\0+$/, '') === MOUTH_SHAPE_PARAMETER_ID
    ? [0, 0]
    : Array.isArray(parameter.defaults)
      ? parameter.defaults
      : [0, 0];
      const definition = {
       id: parameter.name.replace(/\0+$/, ''),
    ...
    const mouthTest = parameterById.get("Param #1");

document.body.insertAdjacentHTML(
  "beforeend",
  `<pre id="mouth-debug" style="
    position:fixed;
    z-index:999999;
    top:10px;
    left:10px;
  ...
  ">${
    mouthTest
      ? JSON.stringify({
          id: mouthTest.id,
          isVec2: mouthTest.isVec2,
          default: mouthTest.defaultValue,
        }, null, 2)
      : "Param #1 NOT FOUND"
  }</pre>`
);
...
const setParameterVectorValue = (parameterId, valueX, valueY) => {
  ...
  // 口パラメータの書き込みをコンソールで確認
  if (parameterId === MOUTH_SHAPE_PARAMETER_ID) {
    console.log(
      '[MOUTH WRITE]',
      parameterId,
      'X=', valueX,
  ...
  if (
    ...
    typeof runtime.set_parameter_vec2 === 'function'
  ) {
  ...
  if (canvas) {
    canvas.dataset.inochi2dTestParameter =
      `${parameterId}:${valueX.toFixed(3)},${valueY.toFixed(3)}`;
  }
};
...
const setPostPhysicsParameterVectorValue = (
  ...
  if (parameterId === MOUTH_SHAPE_PARAMETER_ID) {
    ...
    if (canvas) {
      canvas.dataset.inochi2dMouthDebug =
        `MOUTH ${valueX.toFixed(3)}, ${valueY.toFixed(3)}`;
    }
  }
  ...
  if (
    parameterHandle !== null &&
    typeof runtime.set_post_physics_parameter_vec2_by_handle ===
      'function'
  ) {
...
// リップシンクが閉じているときは、ランタイム更新後にも口を閉じる
　　　　if (
        parameterById.has(MOUTH_SHAPE_PARAMETER_ID) &&   
　　　　  lipSyncLayer.currentOpen <= LIP_SYNC_CLOSE_EPSILON
　　　　) {
      const neutralPose = MOUTH_VISEME_POSES.neutral;
...
if (runtime && mouthTest) {
  setParameterVectorValue("Param #1", 0, 0);
  console.log("MOUTH FORCED CLOSED");
}

　　　　if (canvas) {
       canvas.dataset.inochi2dMouthShapeExists =
       String(parameterById.has("Mouth:: Shape"));
　　　　}
...
getParameterNames() {
  return [...parameterById.keys()];
},
getParameterDebugInfo() {
  return [...parameterById.entries()].map(
    ([id, parameter]) => ({
      id,
      isVec2: parameter.isVec2,
...
async resize(nextWidth, nextHeight, nextDevicePixelRatio) {
  ...
  if (canvas) {
    ...
    canvas.dataset.inochi2dDevicePixelRatio =
