...
export const createInochi2DController = async ({ wasmUrl, debug }) => {
  ...
  const mouthParam = parameters.find(
    (parameter) => parameter?.name === "Mouth:: Shape"
  );

  const parameterInfo = parameters.map((parameter) => ({
  name: parameter?.name,
  uuid: parameter?.uuid,
  is_vec2: parameter?.is_vec2,
  defaults: parameter?.defaults,
}));

const status = document.createElement("pre");

const parameterSummary = parameters.map((parameter) => ({
  name: parameter?.name,
  uuid: parameter?.uuid,
  is_vec2: parameter?.is_vec2,
  min: parameter?.min,
  max: parameter?.max,
  defaults: parameter?.defaults,
  bindingCount: Array.isArray(parameter?.bindings)
...
const parameterSummary = parameters.map((parameter) => ({
  ...
  min: parameter?.min,
  max: parameter?.max,
  bindings: Array.isArray(parameter?.bindings)
...
status.textContent =
  "PARAMETER BINDINGS:\n" +
  JSON.stringify(parameterSummary, null, 2);
